...
class ConvLayer(torch.nn.Module):
    ...
    def forward(self, data, x):
        ...
        if self.use_norm_gate:
            ...
            s0 = torch.cat(
                [x[edge_dst][:, self.irrep_in_node.slices()[0]], x[edge_dst][:, self.irrep_in_node.slices()[0]], s0],
                dim=-1,
            )

        self_x = x
        edge_features = self.tp_node(x[edge_src], data.vec_feat_sh, self.fc_node(data.dis_feat) * self.layer_l0(s0))

        if self.edge_wise:
            out = edge_features
        else:
            out = scatter(edge_features, edge_dst, dim=0, dim_size=len(x))
            # [bs * n_atom, 3200] [bs * n_atom * (n_atom - 1), 3200] [bs * n_atom * (n_atom - 1)] [bs * n_atom]
            # warning: only last edge_feature of each atoms is surve as node embedding
...
class ConvNetLayer(torch.nn.Module):
    def __init__(
        ...
        self.diag_lin = Linear(irrep_hami, self.irrep_in_node)
        self.non_diag_lin = Linear(irrep_hami, self.irrep_in_node)

    def forward(self, data, x, diag_feat, non_diag_feat):
        x_hami = self.diag_lin(diag_feat) + scatter(
            self.non_diag_lin(non_diag_feat), data.edge_index[0], dim=0, dim_size=len(diag_feat)
        )
        old_x = x
        x = self.conv(data, x + x_hami)
...
class PairNetLayer(torch.nn.Module):
    ...
    def forward(self, data, node_attr, node_pair_attr=None):
        ...
        node_pair = self.tp_node_pair(
            node_attr[src], node_attr[dst], self.fc_node_pair(data.dis_feat_all) * self.fc(s0)
        )
...
class SelfNetLayer(torch.nn.Module):
    def __init__(
        ...
        self.irrep_tp_in_node, _ = get_feasible_irrep(
            self.irrep_bottle_hidden, o3.Irreps("0e"), self.irrep_bottle_hidden
        )
        self.irrep_tp_out_node, instruction_node = get_feasible_irrep(
            self.irrep_bottle_hidden, self.irrep_bottle_hidden, self.irrep_bottle_hidden, tp_mode="uuu"
        )

        # - Build modules -
        self.linear_node_1 = Linear(
            irreps_in=self.irrep_in_node,
            irreps_out=self.irrep_bottle_hidden,
            internal_weights=True,
            shared_weights=True,
            biases=True,
        )
    ...
    def forward(self, data, x, old_fii):
        ...
        x = self.tp(xl, xr)
        if self.resnet and x.shape == old_x.shape:
            x = x + old_x
        x = self.norm_gate(x)
    ...
    def forward(self, x_in, weights=None, bias_weights=None):
        ...
        if len(self.irrep_in) == 1:
            ...
            for i, mul_ir in zip(self.irrep_in.slices(), self.irrep_in):
                if mul_ir[1].p == 1:
                    x_in_s.append(x_in[:, i].reshape(batch_num, mul_ir.mul, mul_ir.ir.dim))
                    irreps_in.append(mul_ir)

        outputs = {}
        flat_weight_index = 0
        bias_weight_index = 0
        for ins in self.instructions:
            mul_ir_in = irreps_in[ins[0]]
            ...
            w3j_matrix = o3.wigner_3j(ins[1], ins[2], ins[0]).type_as(x1)
        ...
        for i in range(len(self.irrep_out_1)):
            ...
            for j in range(len(self.irrep_out_2)):
                if (i, j) not in outputs:
                    blocks += [
                        torch.zeros(
                            (x_in.shape[0], self.irrep_out_1[i].dim, self.irrep_out_2[j].dim), device=x_in.device
                        ).type(x_in.type())
                    ]
                else:
                    blocks += [outputs[(i, j)]]
            ...
            for j, (num_out1, ir_out1) in enumerate(irrep_out_1):
                for k, (num_out2, ir_out2) in enumerate(irrep_out_2):
                    if ir_in in ir_out1 * ir_out2:
                        instructions.append([ir_in.l, ir_out1.l, ir_out2.l, True, 1.0, [num_in, num_out1, num_out2]])
        return instructions
