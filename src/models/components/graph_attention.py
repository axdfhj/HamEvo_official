# Derived from Equiformer (https://github.com/atomicarchitects/equiformer)
# Copyright (c) 2023 Yi-Lun Liao, licensed under the MIT License.
# See THIRD_PARTY_NOTICES in the project root for details.
...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.nonlinear_message:
            ...
            self.vec2heads = Vec2AttnHeads((o3.Irreps(f"{mul_alpha_head}x0e") + irreps_head).simplify(), num_heads)
...
class FeedForwardNetwork(torch.nn.Module):
    ...
    def __init__(self, irreps_node_input, irreps_node_attr, irreps_node_output, irreps_mlp_mid=None, proj_drop=0.1):
...
class NodeEmbeddingNetwork(torch.nn.Module):
    def __init__(self, irreps_node_embedding, max_atom_type=_MAX_ATOM_TYPE, bias=True, dtype="float32"):

        super().__init__()
        self.max_atom_type = max_atom_type
        self.irreps_node_embedding = o3.Irreps(irreps_node_embedding)
        self.atom_type_lin = LinearRS(o3.Irreps(f"{self.max_atom_type}x0e"), self.irreps_node_embedding, bias=bias)
    ...
    def forward(self, node_atom):
        ...
        """
        node_atom_onehot = torch.nn.functional.one_hot(node_atom, self.max_atom_type).to(self.dtype)
