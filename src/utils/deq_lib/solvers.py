...
def anderson(
    ...
    **kwargs,
    ...
    Ham_iteration_list.append(x0)
    Ham_iteration_list.append(F[:, 0].reshape_as(x0).clone())

    X[:, 1], F[:, 1] = F[:, 0], f(F[:, 0].reshape_as(x0)).reshape(bsz, -1)
    Ham_iteration_list.append(F[:, 1].reshape_as(x0).clone())
    # print(f"Step-0-mae: {torch.mean(torch.abs(F[:,0] - X[:,0]))}")
    # print(f"Step-1-mae: {torch.mean(torch.abs(F[:,1] - X[:,1]))}")
    # save_list = [torch.mean(torch.abs(F[:,0] - X[:,0])), torch.mean(torch.abs(F[:,1] - X[:,1]))]
    ...
    trace_dict = {"abs": [], "rel": []}
    lowest_dict = {"abs": 1e10, "rel": 1e10}
    lowest_step_dict = {"abs": 0, "rel": 0}

    # init lowest_xest and lowest_gx
    lowest_xest = X[:, 0].view_as(x0).clone().detach()
    lowest_gx = (F[:, 0] - X[:, 0]).view_as(x0).clone().detach()
    for k in range(2, threshold):
        ...
        try:
            ...
            alpha = (torch.linalg.pinv(H[:, : n + 1, : n + 1]) @ y[:, : n + 1])[:, 1 : n + 1, 0]

        # cond_number = torch.linalg.cond(H[:, :n+1, :n+1])
        # if torch.any(cond_number > threshold_value):
        #     print(f"Max condition number is {cond_number.max()}. solve with linalg.pinv")
        #     # 使用伪逆或者添加更多的正则化
        #     alpha = (torch.linalg.pinv(H[:, :n+1, :n+1]) @ y[:, :n+1])[:, 1:n+1, 0]
        # else:
        #     print(f"Max condition number is {cond_number.max()}.")
        #     alpha = torch.linalg.solve(H[:, :n+1, :n+1], y[:, :n+1])[:, 1:n+1, 0]   # (bsz x n)

        X[:, k % m] = beta * (alpha[:, None] @ F[:, :n])[:, 0] + (1 - beta) * (alpha[:, None] @ X[:, :n])[:, 0]
        F[:, k % m] = f(X[:, k % m].reshape_as(x0)).reshape(bsz, -1)
        Ham_iteration_list.append(F[:, k % m].reshape_as(x0).clone())
        gx = (F[:, k % m] - X[:, k % m]).view_as(x0)
        abs_diff = gx.norm().item()
        rel_diff = abs_diff / (1e-5 + F[:, k % m].norm().item())
        # print(f"Step {k}: abs_diff: {abs_diff}, rel_diff: {rel_diff}, mae: {torch.mean(torch.abs(F[:,k%m] - X[:,k%m]))}")
        # save_list.append(torch.mean(torch.abs(F[:,k%m] - X[:,k%m])))
        # print("alpha", alpha)
        # print(torch.bmm(G[:1],G[:1].transpose(1,2)))
        # print("valid error.", gx.abs().mean())
        diff_dict = {"abs": abs_diff, "rel": rel_diff}
        trace_dict["abs"].append(abs_diff)
        trace_dict["rel"].append(rel_diff)
    ...
    if return_F:
        out["F"] = F
    X = F = None
    out["Ham_iteration_list"] = Ham_iteration_list
    return out
