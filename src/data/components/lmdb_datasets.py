...
def _resolve_lmdb_source(processed_dir: str, lmdb_name: str, manifest_name: str | None = None):
    """Resolve either a release shard manifest or a legacy single LMDB path."""
    if manifest_name is None:
        manifest_name = f"{osp.splitext(lmdb_name)[0]}-shards.json"

    manifest_path = osp.join(processed_dir, manifest_name)
    if osp.exists(manifest_path):
        return "sharded", load_lmdb_shards(manifest_path), manifest_name

    lmdb_path = osp.join(processed_dir, lmdb_name)
    if osp.exists(lmdb_path):
        return "single", lmdb_path, f"{lmdb_name}/data.mdb"

    raise FileNotFoundError(f"Could not find {lmdb_path} or {manifest_path}")


def _count_lmdb_source(source_kind: str, source) -> int:
    return count_lmdb_shard_entries(source) if source_kind == "sharded" else count_lmdb_entries(source)


def _read_lmdb_source(source_kind: str, source, idx: int):
    return read_lmdb_shards(source, idx) if source_kind == "sharded" else read_lmdb(source, idx)
...
class QH9Loop(BaseHamiltonianDataset):
    def __init__(
        self,
        root: str | Path = "scf_dataset/qh9-scf-rerun",
        full_orbitals: int = 14,
        transform: Callable | None = None,
        ...
        self.lmdb_name = f"qh9{self.mode}.lmdb"
        self.manifest_name = f"qh9{self.mode}-shards.json"
        self.source_kind, self.lmdb_source, self.processed_name = _resolve_lmdb_source(
            os.path.join(self.folder, "processed"),
            self.lmdb_name,
            self.manifest_name,
        )
        super().__init__(root, transform, pre_transform, pre_filter)

        self.n_conformations = _count_lmdb_source(self.source_kind, self.lmdb_source)
        log.info(f"total conformation number is {self.n_conformations}.")
    ...
    @property
    def processed_file_names(self):
        if self.source_kind == "sharded":
            return [f"{shard['path'].split(os.sep)[-1]}/data.mdb" for shard in self.lmdb_source]
        return [self.processed_name]

    def _get_raw_data(self, idx):
        data_dict = _read_lmdb_source(self.source_kind, self.lmdb_source, idx)
...
class GDB17LoopV2(BaseHamiltonianDataset):
    def __init__(
        self,
        root: str | Path = "scf_dataset/gdb17-scf-rerun",
        transform: Callable | None = None,
        pre_filter: Callable | None = None,
        data_type: Literal["float32", "float64"] = "float32",
        ...
        local_dir = osp.join(self.folder, "processed") if self.processed_link is None else self.processed_link
        self.source_kind, self.lmdb_source, self.processed_name = _resolve_lmdb_source(
            local_dir,
            "gdb-17.lmdb",
            "gdb-17-shards.json",
        ...
        weights_path = _first_existing_path(
            [
                os.path.join(root, "n_cycle.pt"),
                os.path.join(root, "split", "n_cycle.pt"),
            ]
        )
        split_path = _first_existing_path(
            [
                os.path.join(root, "gdb17_random_12.pt"),
                os.path.join(root, "split", "gdb-17_random_12.pt"),
            ]
        )
        self.weights = torch.load(weights_path)
        self.cycle_splits = torch.load(split_path)
        self.molecular_splits = self.cycle_splits
