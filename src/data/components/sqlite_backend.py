...
class HamiltonianDatabase:
    ...
    def __getitem__(self, idx: int | list) -> tuple:
        cursor = self._get_connection(flags=apsw.SQLITE_OPEN_READONLY).cursor()
    ...
    def add_data(
        ...
        if self._any_is_nan(Z, R, E, F, H, S, C):
            logger.warning("encountered NaN, data is not added")
            return
    ...
    def __getstate__(self):
        """Custom serialization method to exclude database connections"""
        state = self.__dict__.copy()
        # Exclude database connections; only keep the filename
        if "_connections" in state:
            del state["_connections"]
        return state

    def __setstate__(self, state):
        """Custom deserialization method to restore database connections"""
